...
`packwiz` 是本仓库的 mod 管理工具，用于维护 `*.pw.toml`、下载来源和发布索引，不是游戏启动器。根目录 `pack.toml`、`index.toml`、`.packwizignore` 由 `devtool.bat prepare-pack` 或 packwiz 操作生成，不作为源码提交；源码侧维护 `pack/pack.toml` 和 `pack/.packwizignore.source` 模板。
...
执行后必须人工检查生成的 `*.pw.toml`，不要提交 `mods/*.jar` 或根目录生成的 `index.toml`。
