if (global.hasMod('quality_food')) {
  ServerEvents.tags('block', (event) => {
    const BuiltInRegistries = Java.loadClass('net.minecraft.core.registries.BuiltInRegistries');
    const existingBlocks = (ids) =>
      ids.filter((id) => id.startsWith('#') || global.blockExists(id));

    event.add(
      'quality_food:quality_blocks',
      existingBlocks([
        'brewinandchewin:flaxen_cheese_wheel',
        'vintagedelight:cheese_pizza',
        'vintagedelight:meat_pizza',
        'youkaishomecoming:chest_of_heart_throbbing_surprise',
        'youkaishomecoming:flesh_feast',
        'youkaishomecoming:red_velvet_cake',
        'youkaishomecoming:tarte_lune',
        'dungeonsdelight:glow_berry_gelatin_block',
        'dungeonsdelight:ossobusco_block',
        'dungeonsdelight:guardian_angel_block',
        'dungeonsdelight:spider_pie',
        'dungeonsdelight:monster_cake',
        'ends_delight:dragon_leg_with_sauce_block',
        'ends_delight:grilled_shulker_block',
        'ends_delight:steamed_dragon_egg_block',
        'ends_delight:dragon_meat_stew_block',
        'cavedelight:roasted_grottoceratops',
        'cavedelight:boiled_trilocaris',
        'cavedelight:roasted_dino_chop',
        'cavedelight:pinenut_pie',
        'cavedelight:tectonic_cheesecake',
        'trailandtales_delight:cherry_cake',
        'trailandtales_delight:cherry_cheese_pie',
        'trailandtales_delight:cooked_sniffer_egg_block',
        'trailandtales_delight:stuffed_sniffer_egg_block',
        'farmersrespite:coffee_cake',
        'mynethersdelight:magma_cake',
        'mynethersdelight:ghasta_with_cream',
        'mynethersdelight:roast_stuffed_hoglin',
        'oceanic_delight:sponge_cake',
        'oceanic_delight:stuffed_pufferfish',
        'oceanic_delight:paella_plate',
        'silentsdelight:sculk_catalyst_pie',
      ])
    );

    const foodBlockMods = [
      'vintagedelight',
      'culturaldelights',
      'fruitsdelight',
      'collectorsreap',
      'cratedelight',
      'neapolitan',
      'mynethersdelight',
      'oceanic_delight',
      'frycooks_delight',
      'corn_delight',
      'youkaishomecoming',
      'nethervinery',
      'dungeonsdelight',
      'ends_delight',
      'crabbersdelight',
    ];

    BuiltInRegistries.BLOCK.keySet().forEach((key) => {
      const id = String(key);
      const namespace = id.split(':')[0];
      if (
        (foodBlockMods.includes(namespace) && id.endsWith('_crate')) ||
        id.endsWith('_bag') ||
        id.endsWith('_barrel')
      ) {
        event.add('quality_food:quality_blocks', id);
      }
    });

    event.add(
      'createdelightcore:quality_crops',
      existingBlocks([
        'neapolitan:vanilla_vine',
        'neapolitan:strawberry_bush',
        'neapolitan:small_banana_frond',
        'neapolitan:banana_frond',
        'neapolitan:large_banana_frond',
        'neapolitan:mint',
        'vinery:red_grape_bush',
        'vinery:white_grape_bush',
        'vinery:savanna_grape_bush_red',
        'vinery:savanna_grape_bush_white',
        'vinery:taiga_grape_bush_red',
        'vinery:taiga_grape_bush_white',
        'vinery:jungle_grape_bush_red',
        'vinery:jungle_grape_bush_white',
        'nethervinery:crimson_grape_bush',
        'nethervinery:warped_grape_bush',
        'minecraft:cocoa',
        'farmersdelight:budding_tomatoes',
        'fruitsdelight:bayberry_leaves',
        'fruitsdelight:kiwi_leaves',
        'fruitsdelight:fig_leaves',
        'fruitsdelight:durian_leaves',
        'fruitsdelight:pear_leaves',
        'fruitsdelight:hawberry_leaves',
        'fruitsdelight:lychee_leaves',
        'fruitsdelight:mango_leaves',
        'fruitsdelight:persimmon_leaves',
        'fruitsdelight:peach_leaves',
        'fruitsdelight:orange_leaves',
        'fruitsdelight:apple_leaves',
        'fruitsdelight:mangosteen_leaves',
        'farmersdelight:brown_mushroom_colony',
        'farmersdelight:red_mushroom_colony',
        'mynethersdelight:crimson_fungus_colony',
        'mynethersdelight:warped_fungus_colony',
        'collectorsreap:portobello_colony',
        'minecraft:sweet_berry_bush',
        'collectorsreap:pomegranate_bush',
        'collectorsreap:lime_bush',
        'fruitsdelight:lemon_tree',
        'fruitsdelight:blueberry_bush',
        'fruitsdelight:cranberry_bush',
        'mynethersdelight:powdery_cane',
        'mynethersdelight:powdery_cannon',
        'mynethersdelight:bullet_pepper',
      ])
    );
    event.add(
      'quality_food:quality_blocks',
      existingBlocks(['#createdelightcore:quality_crops', 'minecraft:sea_pickle'])
    );
    event.remove('quality_food:quality_blocks', '#farmersdelight:wild_crops');
  });

  ServerEvents.tags('item', (event) => {
    const existingItems = (ids) => ids.filter((id) => id.startsWith('#') || global.itemExists(id));

    event.remove('quality_food:material_whitelist', '#farmersdelight:wild_crops');
    event.add(
      'quality_food:material_whitelist',
      existingItems([
        'create:dough',
        'farmersdelight:brown_mushroom_colony',
        'farmersrespite:coffee_beans',
        'farmersdelight:red_mushroom_colony',
        'mynethersdelight:crimson_fungus_colony',
        'mynethersdelight:warped_fungus_colony',
        'collectorsreap:portobello_colony',
        'minecraft:brown_mushroom',
        'minecraft:red_mushroom',
        'collectorsreap:portobello',
        'minecraft:crimson_fungus',
        'minecraft:warped_fungus',
        'minecraft:kelp',
        'create:cinder_flour',
        'ratatouille:salty_dough',
        'ratatouille:wheat_kernels',
        'ratatouille:cocoa_solids',
        'createdelightcore:corn_flour',
        'bakeries:foamed_cream',
        'createdelightcore:board_noodles',
        'createdelightcore:wafer_dough',
        'createdelightcore:vermicelli',
        'createdelightcore:raw_cheese_pizza',
        'bakeries:sweet_dough',
        'bakeries:whole_wheat_dough',
        'bakeries:bagel_dough',
        'bakeries:whole_wheat_bagel_dough',
        'bakeries:round_bread_dough',
        // 呃呃啊啊: bakeries:berry_bread_dough is missing for now.
        // 'bakeries:berry_bread_dough',
        'bakeries:brown_sugar_roll_dough',
        'bakeries:pineapple_bun_dough',
        'bakeries:meat_floss_bread_dough',
        'bakeries:croissant_dough',
        'bakeries:salt_croissant_dough',
        'bakeries:mould_toast_dough',
        'bakeries:mould_cheese_cocoa_toast_dough',
        'bakeries:baguette_dough',
        'bakeries:ciabatta_dough',
        'bakeries:focaccia_dough',
        'bakeries:country_bread_dough',
        'ratatouille:cocoa_powder',
        'ratatouille:cocoa_butter',
        'ratatouille:dried_cocoa_beans',
        'ratatouille:dried_cocoa_nibs',
        'create_bic_bit:sweet_dough',
        'create_bic_bit:crushed_nether_wart',
        'casualnessdelight:raw_gluten',
        'casualnessdelight:gluten',
        'casualnessdelight:raw_chicken_bobo_chicken',
        'casualnessdelight:raw_cabbage_bobo_chicken',
        'casualnessdelight:raw_potato_bobo_chicken',
        'casualnessdelight:potato_slice',
        'createcafe:oreo_dough',
        'createcafe:tapioca_flour',
        'dungeonsdelight:gunk',
        'dungeonsdelight:rotbulb',
        'dungeonsdelight:sculk_polyp',
        'dungeonsdelight:spider_extract',
        'dungeonsdelight:rancid_reduction',
        'dungeonsdelight:ghastly_spirits',
        'dungeonsdelight:bloody_mary',
        'dungeonsdelight:taro_milk_tea',
        'dungeonsdelight:ancient_egg',
        'dungeonsdelight:sculk_mayo_block',
        'dungeonsdelight:spider_pie',
        'dungeonsdelight:sculk_tart',
        'dungeonsdelight:monster_cake',
        'dungeonsdelight:glow_berry_gelatin_block',
        'dungeonsdelight:ossobusco_block',
        'dungeonsdelight:guardian_angel_block',
        'ends_delight:ender_pearl_grain',
        'ends_delight:dried_chorus_flower',
        'ends_delight:non_hatchable_dragon_egg',
        'ends_delight:dragon_breath_soda',
        'ends_delight:chorus_fruit_wine',
        'festival_delicacies:uncooked_sweet_zongzi',
        'festival_delicacies:uncooked_meat_zongzi',
        'festival_delicacies:bamboo_leaf',
        'festival_delicacies:mugwort',
        'cavedelight:fiddlehead_tea',
        'trailandtales_delight:cherry_petal',
        'trailandtales_delight:dried_cherry_petal',
        'trailandtales_delight:pitcher_plant',
        'farmersrespite:rose_hips',
        'neapolitan:banana_bunch',
        'vintagedelight:oat',
        'crabbersdelight:clam',
        'youkaishomecoming:soybean',
        'youkaishomecoming:pods',
        'youkaishomecoming:udumbara_flower',
        'youkaishomecoming:mandrake_root',
        'youkaishomecoming:matcha',
        'youkaishomecoming:stripped_mandrake_root',
        'youkaishomecoming:dried_mandrake_flower',
        'collectorsreap:pomegranate',
        'collectorsreap:lime_popsicle',
        'collectorsreap:urchin',
        'brewinandchewin:unripe_scarlet_cheese_wheel',
      ])
    );
  });
}
